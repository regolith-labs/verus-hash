...
// Define ROUND_CONST_COUNT if not already defined (e.g., by haraka_portable.h if it were to define it)
#ifndef ROUND_CONST_COUNT
#define ROUND_CONST_COUNT 40
#endif

// Directly define the Haraka round constants, copied from origin-impl/haraka_portable.c
static const unsigned char HARAKA_ROUND_CONSTANTS[ROUND_CONST_COUNT][16] = {
...
// These are the mutable global arrays for round constants.
// `rc` will be initialized by `load_constants_port`.
// `rc0` will be zero-initialized as it's static.
// `rc_sseed` is used by `tweak_constants` which is not currently part of the SBF path.
static unsigned char rc[ROUND_CONST_COUNT][16];
static unsigned char rc0[ROUND_CONST_COUNT][16]; // Automatically zero-initialized
static unsigned char rc_sseed[ROUND_CONST_COUNT][16]; // For tweak_constants, also zero-initialized
...
void load_constants_port()
{
    ...
    // Ensure verus_memcpy is used if memcpy is not available/safe for BPF
    verus_memcpy(rc, HARAKA_ROUND_CONSTANTS, sizeof(HARAKA_ROUND_CONSTANTS));
    // rc0 is already zero-initialized as a static global.
    // If explicit zeroing is needed for BPF, do it here:
