...
static void haraka512_perm_keyed(unsigned char *out, const unsigned char *in, const u128 *rc);
...
// Removed load_constants_port() definition. tweak_constants is used instead.

// tweak_constants is now the primary function for setting up round constants.
// It's declared in the header and called from verus_hash.cpp.
void tweak_constants(const unsigned char *pk_seed, const unsigned char *sk_seed,
                     unsigned long long seed_length)
{
    // Make buf static to avoid exceeding BPF stack limit (512 bytes)
    // 40 * 16 = 640 bytes.
    // Note: This makes the function non-reentrant.
    static unsigned char buf[40*16];

    /* Use the standard constants to generate tweaked ones. */
    memcpy(rc, haraka_rc, 40*16);

    /* Constants for sk.seed */
    if (sk_seed != (void*)0) { // Replaced NULL with (void*)0 for SBF compatibility
...
// Made static as it's only called internally (seems related to tweak_constants)
static void haraka256_sk(unsigned char *out, const unsigned char *in)
{
    int i, j;
