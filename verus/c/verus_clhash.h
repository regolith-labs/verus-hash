...
#include <cpuid.h>
#include <cpuid.h>
// #include <x86intrin.h> // Removed: Not available for SBF target
#endif // !WIN32

// #include <boost/thread.hpp> // Removed: Not available/needed for SBF
// #include "tinyformat.h" // Removed: Not available/needed for SBF
...
struct thread_specific_ptr {
    ...
    // Destructor logic removed as thread_local might behave differently or be unsupported in SBF.
    // Manual cleanup might be needed if used, but clhash part is removed for SBF build.
    ~thread_specific_ptr() {
        // No-op for SBF, rely on system cleanup if applicable.
        // If this were used, manual resource management would be necessary.
    }
};

// Removed thread_local variables as they are not suitable for SBF's execution model
// extern thread_local thread_specific_ptr verusclhasher_key;
// extern thread_local thread_specific_ptr verusclhasher_descr;

// Removed __cpuverusoptimized and related functions as CLHASH path is disabled for SBF
// extern int __cpuverusoptimized;

// Removed CLHASH function declarations as they are not used in the portable SBF path
...
// Always return false for SBF as optimized path is disabled
...
// Removed CLHASH function declarations
...
// Removed alloc_aligned_buffer declaration, use standard allocation if needed, but likely unused now.
...
// Removed includes that are not SBF compatible
// #include <vector>
// #include <string>
// #include <iostream>

// Removed helper functions depending on std::string/stringstream
/*
template <typename T>
...
// Removed verusclhasher struct as it's tied to the optimized CLHASH path
/*
