        ...
        if (nNet > 0)
        {
            ...
            if (CBitcoinAddress(rec->address).IsValid())
            {
                ...
                if (wallet->mapAddressBook.count(address))
                {
                    ...
                    isminetype whatType = ::IsMine(*wallet, address);
                    QString addressOwned = (whatType == ISMINE_SPENDABLE) ? tr("own address") : (whatType == ISMINE_SHARED) ? tr("shared-ownership") : tr("watch-only");
